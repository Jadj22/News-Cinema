...
import { Link, NavLink } from 'react-router-dom';  // Importation des composants de navigation
import { LanguageContext } from '../context/LanguageContext'; // Importation du contexte de langue

const Header = () => {
  // Initialisation de l'état pour gérer l'affichage mobile du menu
  const [hidden, setHidden] = useState(true);

  // Utilisation du contexte de langue
  const { language, changeLanguage } = useContext(LanguageContext);

  // Initialisation du mode sombre, avec récupération depuis localStorage ou valeur par défaut "true"
  const [darkMode, setDarkMode] = useState(() => {
    const savedMode = localStorage.getItem("darkMode");
    try {
      return savedMode ? JSON.parse(savedMode) : true;
    } catch (e) {
      return false;  // Retourne false si le parsing échoue
    }
  });
  
  // Hook useEffect pour sauvegarder l'état du mode sombre dans localStorage à chaque changement de celui-ci
  useEffect(() => {
  ...
  // Classes CSS pour les liens actifs et inactifs du menu de navigation
  ...
  return (
    <header>
        ...
        <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse">
            <img src={Logo} className="h-8" alt="News Cinema Logo" />
        ...
        <button onClick={() => setDarkMode(!darkMode)} type="button" className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg mr-1 hover:bg-gray-100 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700">
            ...
            <ul className="flex text-left flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                <li>
                ...
                <NavLink to="/movies/top" className={({isActive}) => isActive ? activateClass: unActivateClass}>
                  {language === 'fr' ? 'Top Notés' : 'Top Rated'}
                </NavLink>
                </li>
                <li>
